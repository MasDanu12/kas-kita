...
## Jika Database D1 SUDAH TERISI DATA LAMA

Jangan menjalankan ulang seluruh `schema.sql` pada database produksi lama. Gunakan `MIGRASI_D1_EXISTING.sql` terlebih dahulu.

Di D1 Console, jalankan perintah `ALTER TABLE` pada BLOK 1 satu per satu. Jika muncul `duplicate column name`, lewati perintah untuk kolom tersebut. Setelah itu lanjutkan BLOK 2, BLOK 3, dan BLOK 4. Migrasi ini tidak menghapus transaksi lama.

Setelah migrasi selesai, deploy `worker.js` dan isi asset terbaru.
