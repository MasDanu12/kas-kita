...
async function handleApi(request, env, path, url) {
  ...
  if (path === '/api/transaksi' && method === 'POST') {
    const { tipe, kategori, jumlah, catatan, metode, tanggal, akun_id, akun_tujuan_id, anggota_id, sumber } = await request.json();
    ...
    let anggota = null;
    const trxSumber = tipe === 'masuk' ? (sumber === 'anggota' ? 'anggota' : 'umum') : 'umum';
    if (tipe === 'masuk' && trxSumber === 'anggota') {
      if (!anggota_id) return err('Anggota wajib dipilih untuk pemasukan dari anggota');
      anggota = await env.DB.prepare('SELECT id, nama FROM anggota WHERE id = ? AND organization_id = ? AND aktif = 1').bind(anggota_id, orgId).first();
      if (!anggota) return err('Anggota tidak ditemukan atau tidak aktif', 404);
    }
    const id = newId();
    const tgl = tanggal || todayStr();
    await env.DB.prepare(
      `INSERT INTO transaksi (id, organization_id, tipe, sumber, kategori, jumlah, catatan, metode, akun_id, akun_tujuan_id, anggota_id, tanggal, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(id, orgId, tipe, trxSumber, kategori || null, jumlah, catatan || null, metode || null, akun_id || null, akun_tujuan_id || null, anggota?.id || null, tgl, userId).run();
    const org = await env.DB.prepare('SELECT nama FROM organizations WHERE id = ?').bind(orgId).first();
    return json({ transaksi: { id, tipe, kategori, jumlah, catatan, tanggal: tgl, organisasi: org.nama, anggota_nama: anggota?.nama || null } });
  }
